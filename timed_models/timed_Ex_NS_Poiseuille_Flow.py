...
# %% [markdown]
# # Navier Stokes benchmark: Poiseuille 2D flow
# - imposed velocity B.C. from https://www.fifty2.eu/innovation/planar-poiseuille-flow-2-d-in-preonlab/
...
# %%
expt_name = f"Poiseuille-res{resolution}-order{order}-{mesh_type}-case{case_num}"
...
for step in range(0, maxsteps):
    ...
    # if step > 0:
    #     t_diff, t_adv = navier_stokes.estimate_dt() # if step == 0, velocity field is not yet evaluated inside
    #     delta_t = t_adv
    # else:
    #     delta_t = delta_t # calculated beforehand

    # constant timestep for now
    delta_t = delta_t
    ...
    if uw.mpi.rank == 0:
        ...
        # if step > 0:
        #     print(f"t_use: {delta_t}, t_adv: {t_adv}, t_diff:{t_diff}")
