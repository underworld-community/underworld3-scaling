...
# %% [markdown]
# ## Timed model including options for MMS examples from petsc
# - SolDB (2D or 3D) - currently cannot solve
...
# only supported for Stokes equation solving
# solDB, petscMMS1, petscMMS2
soln_name = "petscMMS2"
...
mesh = uw.meshing.UnstructuredSimplexBox(regular=False,
    minCoords=(0.0, 0.0, 0.0) if dim == 3 else (0.0, 0.0),
    maxCoords=(1.0, 1.0, 1.0) if dim == 3 else (1.0, 1.0),
    cellSize=1 / n_els,
...
# %% [markdown]
# # SolDB set-up
# SolDB is test case 4 from the Dohrmann and Bochev 2004.
# A unit cube has the following velocity and pressure fields:
#
# \begin{equation}
# u_1 = x + x^2 + xy + x^3y
# \end{equation}
...
#
# \begin{equation}
# p = xyz + x^3y^3z - \dfrac{5}{32}
# \end{equation}
#
# From the Stokes equation, the body force is calculated by:
#
# \begin{equation}
# \vec S = -\nabla \cdot [\eta(\nabla \vec u + \nabla \vec u^T)]  + \nabla p
# \end{equation}
...
# %%
# solve for the body force and boundary conditions with sympy
from sympy import Matrix, diff, cos, sin, pi

if dim == 3:
    x,y, z = mesh.X
    if soln_name == "solDB":
        u1_an   = x + x**2 + x*y + x**3*y
        u2_an   = y + x*y + y**2 + x**2*y**2
        u3_an   = -2*z - 3*x*z - 3*y*z - 5*x**2*y*z
        p_an    = x*y*z + x**3*y**3*z - 5/32
    elif soln_name == "petscMMS1":
        u1_an   = 2*x**2 + y**2 + z**2
        u2_an   = 2*x**2 - 2*x*y
        u3_an   = 2*x**2 - 2*x*z
        p_an    = x + y + z - 3/2
    elif soln_name == "petscMMS2":
        u1_an   = 2*sin(pi*x) + sin(pi*y) + sin(pi*z)
        u2_an   = -pi*cos(pi*x)*y
        u3_an   = -pi*cos(pi*x)*z
        p_an    = sin(2*pi*x) + sin(2*pi*y) + sin(2*pi*z)
    ...
    if soln_name == "solDB":
        ...
        u1_an   = sin(pi*x) + sin(pi*y)
        u2_an   = -pi*cos(pi*x)*y
        p_an    = sin(2*pi*x) + sin(2*pi*y)
...
# %%
if dim == 3:
    u_vec = Matrix([u1_an, u2_an, u3_an])
    Y = Matrix([x, y, z])
elif dim == 2:
    u_vec = Matrix([u1_an, u2_an])
    Y = Matrix([x, y])

u_jac = u_vec.jacobian(Y)
u_jac_trans = u_jac.transpose()

u_jac_sum = u_jac + u_jac_trans
...
# %%
# I don't know how to use sympy directly to calculate the
# divergence of a tensor
# for now, I will calculate it semi-manually - see notes
# div_u_jac_sum_rN = # divergence of tensor row N

if dim == 3:
    div_u_jac_sum_r0 = diff(u_jac_sum[0, 0], Y[0]) + diff(u_jac_sum[1, 0], Y[1]) + diff(u_jac_sum[2, 0], Y[2])
    div_u_jac_sum_r1 = diff(u_jac_sum[0, 1], Y[0]) + diff(u_jac_sum[1, 1], Y[1]) + diff(u_jac_sum[2, 1], Y[2])
    div_u_jac_sum_r2 = diff(u_jac_sum[0, 2], Y[0]) + diff(u_jac_sum[1, 2], Y[1]) + diff(u_jac_sum[2, 2], Y[2])

    div_u_jac_sum = Matrix([div_u_jac_sum_r0, div_u_jac_sum_r1, div_u_jac_sum_r2])

    grad_p = Matrix([diff(p_an, Y[0]), diff(p_an, Y[1]), diff(p_an, Y[2])]) # gradient of pressure
elif dim == 2:
    div_u_jac_sum_r0 = diff(u_jac_sum[0, 0], Y[0]) + diff(u_jac_sum[1, 0], Y[1])
    div_u_jac_sum_r1 = diff(u_jac_sum[0, 1], Y[0]) + diff(u_jac_sum[1, 1], Y[1])

    div_u_jac_sum = Matrix([div_u_jac_sum_r0, div_u_jac_sum_r1])

    grad_p = Matrix([diff(p_an, Y[0]), diff(p_an, Y[1])]) # gradient of pressure

bf_an = -div_u_jac_sum + grad_p # calculate the body force

#display(div_u_jac_sum)
#display(grad_p)
#display(bf_an)
...
if dim == 3:
    stokes.add_dirichlet_bc(u_vec, "Bottom")
    stokes.add_dirichlet_bc(u_vec, "Top")
    stokes.add_dirichlet_bc(u_vec, "Left")
    stokes.add_dirichlet_bc(u_vec, "Right")
    stokes.add_dirichlet_bc(u_vec, "Front")
    stokes.add_dirichlet_bc(u_vec, "Back")
elif dim == 2:
    stokes.add_dirichlet_bc(u_vec, "Bottom")
    stokes.add_dirichlet_bc(u_vec, "Top")
    stokes.add_dirichlet_bc(u_vec, "Left")
...
# %%
# stokes.petsc_options["snes_monitor"]= True
# stokes.petsc_options["ksp_monitor"] = True
...
if uw.mpi.size == 1 and show_plot:
    ...
    pvmesh.point_data["Vmag"] = uw.visualisation.scalar_fn_to_pv_points(pvmesh, v_soln.sym[2])
    ...
    arrows = pl.add_arrows(velocity_points.points, velocity_points.point_data["V"], mag=0.01, opacity=1, show_scalar_bar=False)
...
# %%
stokes.bodyforce = bf_an

#display(stokes.bodyforce)
...
#if max_its >= 0:
#    # define velocity sympy function - only for the first 2 coordinates
#    v_fn = sympy.Matrix(
#    [
#        Piecewise(
#            (-1, ((x >= 0.5) & (y >= 0.5))),
...
#        v_soln.data[:, 0:2] = uw.function.evaluate(v_fn, v_soln.coords[:, 0:2])
#
...
#swarm.advection(v_soln.fn, 0.01 * stokes.estimate_dt())
#if uw.mpi.rank == 0 and debug:
...
outputPath = "../../../../../scratch/el06/jg0883/uw3-scaling/output/timing-tests-petscMMS2-3D/"
...
#
#swarm.write_timestep(f"timing_els_{n_els}", "swarm", swarmVars = [], outputPath = outputPath, index=0)
#
#if uw.mpi.rank == 0 and debug:
